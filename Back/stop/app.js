...
exports.stopHandler = async function (event, context, callback) {
    var recursos = [];
    var errores = [];
    ...
    try {
        for (var recurso of recursos) {
            if (event.httpMethod == 'POST') {
                ...
                await recurso.ApagarPorTag(body.tagName, body.tagValue);
        ...
        if (errores.length !== 0) {
            ...
            callback(null, {
                ...
                "body": JSON.stringify({
                    'message': 'Los recursos han sido apagados exitosamente'
                })
            })
        }
    }
    catch (err) {
        console.log("Ha ocurrido un error al apagar recursos: ", err);
        callback(null, {
            "statusCode": 500,
            "headers": {
                "Content-type": "application/json",
