...
exports.disableScheduleHandler = async function (event, context, callback) {
    var errores = [];
    ...
    try {
       ...
       await cloudWatch.Programado(expressionTurnOn, reglaEncendido, descripcionEncendido, funcionEncendido, status);
        //Programado de Apagado
       await cloudWatch.Programado(expressionTurnOff, reglaApagado, descripcionApagado, funcionApagado, status);
        errores = cloudWatch.errores;
        callback(null,
            {
                "statusCode": 200,
                "headers": {
                ...
                "body": JSON.stringify({
                    'message': 'El deshabilitado de programado de recursos ha sido exitoso',
                    'statusEncendido': status,
                    'statusApagado': status
                })
            })
    }
    catch (err) {
        console.log("Ha ocurrido un error la deshabilitar el programado: ", err);
        callback(null,
            {
                "statusCode": 500,
                "headers": {
